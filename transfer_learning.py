...
Original file is located at
    https://colab.research.google.com/drive/1yjilmYH4PNSbLLTuzwuunManUWPmXrig
...
# Caminhos das pastas
pasta_laranja = '/content/drive/MyDrive/data/orange'
pasta_banana = '/content/drive/MyDrive/data/banana'

# Escolhe uma pasta aleatória
pasta_escolhida = random.choice([pasta_laranja, pasta_banana])

# Lista arquivos na pasta escolhida
...
imagem_path = os.path.join(pasta_escolhida, arquivo_escolhido)
...
imagem_expandida = np.expand_dims(imagem_normalizada, axis=0)

# Faz a predição
predicoes = model_new.predict(imagem_expandida)

# Interpreta a predição
classe_prevista = np.argmax(predicoes)
rotulos = ['Banana', 'Laranja']
nome_classe_prevista = rotulos[classe_prevista]

# Desenha retângulo e etiqueta
altura, largura, _ = imagem.shape
x, y, w, h = int(largura/4), int(altura/4), int(largura/2), int(altura/2)

cv2.rectangle(imagem, (x, y), (x + w, y + h), (0, 255, 0), 2)
cv2.putText(imagem, nome_classe_prevista, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

# Mostra a imagem com Matplotlib
plt.imshow(cv2.cvtColor(imagem, cv2.COLOR_BGR2RGB))
plt.title(f'Reconhecimento: {nome_classe_prevista}')
plt.axis('off')
plt.show()

print(f'A classe prevista é: {nome_classe_prevista}')